...
SENSOR_TYPE_ALIASES = {
    'camera': {'camera'},
    'rgbd_camera': {'rgbd_camera', 'depth_camera'},
}
...
def _normalize_sensor_type(sensor_type):
    ...
    if isinstance(sensor_type, str):
        return sensor_type.strip().lower()
    enum_name = getattr(sensor_type, 'name', None)
    if isinstance(enum_name, str):
        return enum_name.strip().lower()
    if callable(enum_name):
        value = enum_name()
        if isinstance(value, str):
            return value.strip().lower()
    return str(sensor_type).strip().split('.')[-1].lower()


def _sensor_type_matches(sensor_type, canonical_name):
    normalized = _normalize_sensor_type(sensor_type)
    aliases = SENSOR_TYPE_ALIASES.get(canonical_name, {canonical_name})
...
class Model:
    ...
    def payload_bridges_impl(self, world_name, payloads):
        ...
        for sensor_name, value in payloads.items():
            ...
            if _sensor_type_matches(sensor_type, 'camera'):
            ...
            elif _sensor_type_matches(sensor_type, 'rgbd_camera'):
                ros_sensor_prefix = f'sensors/cameras/{sensor_name}'
                nodes.append(Node(
                    package='vrx_ros',
                    executable='optical_frame_publisher',
    ...
    def set_color(self, color):
        # Optional RGBA for the hull. Accepts [r,g,b], [r,g,b,a] or "r g b".
        if isinstance(color, str):
            values = [float(v) for v in color.replace(',', ' ').split()]
        elif isinstance(color, (list, tuple)):
            values = [float(v) for v in color]
        else:
            raise RuntimeError(f'Invalid color type for {self.model_name}: {type(color)}')

        if len(values) not in (3, 4):
            raise RuntimeError(
                f'Color for {self.model_name} must have 3 or 4 values, got {len(values)}'
            )

        # Also support 0-255 style color definitions.
        ...
        self.color = [min(1.0, max(0.0, v)) for v in values]

    def _format_rgba(self, rgba):
        return ' '.join(f'{v:.3f}'.rstrip('0').rstrip('.') for v in rgba)

    def _apply_hull_color(self, model_sdf):
        if not self.color:
            return model_sdf

        rgba = self._format_rgba(self.color)
        specular = self._format_rgba([0.15, 0.15, 0.15, self.color[3]])
        visual_name = re.escape(f'{self.model_name}/base_link_visual')
        visual_pattern = re.compile(
            rf"(<visual name=['\"]{visual_name}['\"]>.*?<material>)(.*?)(</material>)",
            re.DOTALL,
        )

        recolored_sdf = visual_pattern.sub(
            rf"\1\n          <ambient>{rgba}</ambient>\n          "
            rf"<diffuse>{rgba}</diffuse>\n          <specular>{specular}</specular>\n        \3",
            model_sdf,
            count=1,
        )

        if recolored_sdf == model_sdf:
            print(f'Could not find base_link visual for {self.model_name}, color not applied.')

        return recolored_sdf
    ...
    def xacro_cmd(self):
        ...
        # VRX default sensor preset does not enable camera streams.
        # Force-enable the front camera so image topics are available in ROS.
    ...
    def generate(self):
        ...
        model_sdf = self._apply_hull_color(model_sdf)
    ...
    def payload_from_sdf(self, model_sdf):
        ...
        if sdf is not None:
            ...
            for plugin in plugins:
                if plugin.name() == 'gz::sim::systems::Thruster':
                    ...
                    payload['thruster_thrust_' + name] = ['', name]
                elif plugin.name() == 'gz::sim::systems::JointPositionController':
                    name = self.name_from_plugin(plugin.__str__())
                    payload['thruster_rotate_' + name] = ['', name]
                else:
                    payload[plugin.name()] = ['', plugin.filename()]
            return payload

        root = ET.fromstring(model_sdf)
        model_elem = root if _local_name(root.tag) == 'model' else None
        ...
        for link in model_elem:
            if _local_name(link.tag) != 'link':
                continue
            link_name = link.attrib.get('name', '')
            for sensor in link:
                if _local_name(sensor.tag) != 'sensor':
                    continue
                sensor_name = sensor.attrib.get('name')
                if not sensor_name:
                    continue
                payload[sensor_name] = [link_name, sensor.attrib.get('type', '')]

        for plugin in model_elem:
            if _local_name(plugin.tag) != 'plugin':
                continue
            plugin_name = plugin.attrib.get('name', '')
            plugin_filename = plugin.attrib.get('filename', '')
            plugin_sdf = ET.tostring(plugin, encoding='unicode')
            if plugin_name == 'gz::sim::systems::Thruster':
                name = self.name_from_plugin(plugin_sdf)
                if name:
                    payload['thruster_thrust_' + name] = ['', name]
            elif plugin_name == 'gz::sim::systems::JointPositionController':
                name = self.name_from_plugin(plugin_sdf)
                if name:
                    payload['thruster_rotate_' + name] = ['', name]
            elif plugin_name:
                payload[plugin_name] = ['', plugin_filename]
        return payload
