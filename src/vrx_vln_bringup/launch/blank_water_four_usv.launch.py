...
def generate_launch_description():
    ...
    world_default = PathJoinSubstitution(
        [package_share, 'worlds', 'vrx_vln_blank_water.sdf']
    )
    model_config_default = PathJoinSubstitution(
        [package_share, 'config', 'wamv_four_color.yaml']
    )
    ...
    return LaunchDescription([
        DeclareLaunchArgument(
            ...
            description='Absolute path to the blank-water world SDF file.',
        ),
        DeclareLaunchArgument(
            'model_config',
            default_value=model_config_default,
            description='Absolute path to the 4-USV model config YAML.',
        ),
        DeclareLaunchArgument(
            'rviz_config',
